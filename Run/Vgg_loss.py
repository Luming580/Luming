...
class Vgg19_out(nn.Module):
    ...
    def forward(self, X):
        if X.size(1) == 1:  # If the input has 1 channel
            X = X.repeat(1, 3, 1, 1)  # Repeat the channel to make it 3 channels
        elif X.size(1) != 3:
            raise ValueError("Input tensor must have 1 or 3 channels, but got {} channels".format(X.size(1)))
